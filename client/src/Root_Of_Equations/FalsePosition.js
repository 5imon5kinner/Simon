...
export default class FalsePosition extends Component
{
  constructor(props)
    ...
    componentDidMount()
    {
        axios.get('http://localhost:8080/show_FalsePosition.php')//Docker//
        // axios.get('http://localhost/NumericalProject/server/show_FalsePosition.php')
        .then(res=>{
            ...
            res.data.map(dataMap=>{
                let optionsObj = {};
                if(dataMap.EQ_Type === "FalsePosition")
                {
  ...
  getValue = ()=>{
    ...
    EquationFalsePosition = () =>{
      const formData = new FormData();
      formData.append("EqName",this.state.Eq);
      formData.append("EqType","FalsePosition");
  ...
  showResult=()=>{
    ...
    {
      return <div>
        <h5>This is Result of False-Position : {this.state.result}</h5><br/>
